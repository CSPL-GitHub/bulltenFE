...
const FilterComponent = () => {
  ...
  const services = [
    'Rise servers Group',
    'Advance servers Group',
    'Scale servers Group',
    'High Grade servers Group',
    'Game servers Group',
    'Storage servers Group',
    'Soyoustart Servers Group',
    'Kimsufi Server Group',
  ];
  ...
  const handleDiskChange = (disk: string) => {
    if (selectedDisks.includes(disk)) {
      setSelectedDisks(selectedDisks.filter((d) => d !== disk));
    } else {
  ...
  return (
    <div className="p-4 shadow-md rounded-md sm:grid sm:grid-cols-12 gap-10 justify-center items-center">
      ...
      <div className="mb-4 col-span-3 px-5">
        <label className="block text-lg font-semibold text-gray-700 mb-4">Price Range ($)</label>

        <div className="flex justify-between  mb-2">
          <span className="bg-gray-300 px-2 py-1 rounded">${priceRange[0]}</span>
          <span className="bg-gray-300 px-2 py-1 rounded">${priceRange[1]}</span>
        </div>
      ...
      <div className="mb-4 col-span-3 px-5">
        <label className="block text-lg font-semibold text-gray-700 mb-4">RAM</label>
        ...
        <div className="flex justify-between text-xs text-gray-500 mt-2">
          ...
          <span className='text-xl'>1536Gb</span>
        </div>
      </div>
      <div className="px-6 mb-4 col-span-3">
        ...
        <div className="grid grid-cols-2">
          {disks.map((disk, index) => (
            ...
            />
            <label htmlFor={`disk-${index}`} className="text-xl">{disk}</label>
          </div>
          ))}
        </div>
      </div>
