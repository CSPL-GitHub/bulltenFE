...
const WordpressHostingFaqComponent = ({ FaqData }: Props) => {
  ...
  const halfIndex = Math.ceil(FaqData?.faqs.length / 2);
  const firstColumnFaq = FaqData?.faqs.slice(0, halfIndex);
  const secondColumnFaq = FaqData?.faqs.slice(halfIndex);
...
export default WordpressHostingFaqComponent;
