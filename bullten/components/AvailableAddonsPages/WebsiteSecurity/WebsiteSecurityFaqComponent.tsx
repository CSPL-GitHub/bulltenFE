...
const WebsiteSecurityFaqComponent = ({ FaqData }: Props) => {
  ...
  const halfIndex = Math.ceil(FaqData?.FAQ[0]?.faq_list.length / 2);
  const firstColumnFaq = FaqData?.FAQ[0]?.faq_list.slice(0, halfIndex);
  const secondColumnFaq = FaqData?.FAQ[0]?.faq_list.slice(halfIndex);
