            ...
            <div className="border-b  border-gray-200">
                ...
                <div className="grid grid-cols-2 sm:grid-cols-2 lg:grid-cols-4 gap-6 py-8">
                    {tabs.find(tab => tab.title === activeTab)?.content.map((item: any, index: any) => (
                        <div key={index} className="bg-white border border-gray-200 rounded-lg shadow-sm py-4 text-center hover:scale-105">
                            {item.img ? <>  <div className="h-[90px] w-full relative">
                                <Image
                            ...
                            <h3 className="text-lg font-semibold text-gray-800 py-2">{item.heading}</h3>
                        </div>
                    ))}
                </div>
            </div>
