                ...
                <div className=" bg-bullt-quaternary/[0.02] p-4 rounded-md scroll sm:min-h-[400px] h-auto overflow-y-auto">
                    ...
                    <ul className="flex flex-col gap-2">
                        {insideSubMenu?.usecases?.map((useCase: any, index: number) => {
                            return <a href={useCase?.link_url} key={index} className="text-bullt-quaternary hover:bg-bullt-quaternary/[0.05] text-sm hover:underline underline-offset-2 px-1.5 py-1 rounded-sm">{useCase?.link_text}</a>;
                        })}
                    </ul>
                </div> : null}
