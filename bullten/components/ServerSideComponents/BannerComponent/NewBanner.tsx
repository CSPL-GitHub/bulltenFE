...
const NewBanner = ({ banner }: Props) => {
  return (
    <section className="relative min-h-[600px] flex items-center justify-center pt-[70px] -mt-4 container mx-auto rounded-lg"
      style={{
      ...
      <div
          className={`flex-1 flex w-full lg:justify-center mt-8 lg:mt-0 p-14 relative ${banner?.image_position === "Left" ? "sm:flex hidden" : "hidden"}`}>
          <Image
        ...
        <div
          className={`flex-1 flex w-full lg:justify-center mt-8 lg:mt-0  relative ${banner?.image_position === "Right" ? "sm:flex hidden" : "hidden"}`}>
          <Image
            src={`${process.env.NEXT_PUBLIC_BASE_URL}${banner?.image_url}`}
            alt={banner?.alt_text || "Banner Image"}
            layout="responsive"
